// OBD Standards

use crate::{
    consts::{mode01_pids::OBD_STANDARDS_COMPLIANCE, modes::MODE_01},
    parameters::parameters_core::{
        parameters_trait::Parameter,
        parameters_type::{ParameterValue, ParseError},
        parameters_validation::data_validation,
    },
};

pub struct OBDStandards;
impl Parameter for OBDStandards {
    fn mode(&self) -> u8 {
        MODE_01
    }

    fn pid(&self) -> u8 {
        OBD_STANDARDS_COMPLIANCE
    }

    fn parse(&self, data: &[u8]) -> Result<ParameterValue, ParseError> {
        data_validation(data, 3, MODE_01, OBD_STANDARDS_COMPLIANCE)?;
        Ok(ParameterValue::U8(data[2]))
    }
}
